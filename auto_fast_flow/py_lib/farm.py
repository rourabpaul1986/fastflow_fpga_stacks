...
############################################################################################
def one_pipe_farm(host_file, dev_id, farm_name, kernel_name, kernel_index, new_farm):
 code="\n\t//*****Farm Nodes are : "+str(farm_name[:-1])+", no. of pipe: 1"+", Kernel Name : "+kernel_name+", Kernel index : "+kernel_index+" FPGA ID "+str(dev_id)+"*****\n\t"
 ...
 code+="p"+str(farm_name[0])[1:]+str(farm_name[1])[1:]+"->add_stage(new drain(input_l_"+kernel_name+", output_l_"+kernel_name+", "+str(int(kernel_index)-1)+"));\n\t"
...
############################################################################################
############################################################################################     
def mult_pipe_farm(host_file, dev_ids, farm_name, kernel_names, kernel_indexes, new_farm):
 code="\n\t//*Farm Nodes are : "+str(farm_name[:-1])+", no. of pipe: "+str(len(kernel_names))+", Kernel Names : "+str(kernel_names)+", Kernel indexes : "+str(kernel_indexes)+" FPGA IDs "+str(dev_ids)+"\n\t"
 ...
 for i in range(len(kernel_names)):
   ...
   if(i==len(kernel_names)-1):     
      code+="p"+str(farm_name[0])[1:]+str(farm_name[1])[1:]+"->add_stage(new drain(input_l_"+kernel_names[i]+", output_l_"+kernel_names[i]+", "+str(int(kernel_indexes[i])-1)+"));\n\t"
   else:
...
############################################################################################   
def one_pipe_no_farm(host_file, dev_id, farm_name, kernel_name, kernel_index, new_farm):
 code="\n\t//*****Farm Nodes are : "+str(farm_name[:-1])+", no. of pipe: 1"+", Kernel Name : "+kernel_name+", Kernel index : "+kernel_index+"*****\n\t"
...
############################################################################################
############################################################################################    
def mult_pipe_no_farm(host_file, dev_ids, farm_name, kernel_names, kernel_indexes, new_farm):
 code="\n\t//*Farm Nodes are : "+str(farm_name[:-1])+", no. of pipe: "+str(len(kernel_names))+", Kernel Names : "+str(kernel_names)+", Kernel indexes : "+str(kernel_indexes)+" FPGA IDs "+str(dev_ids)+"\n\t"
