...
typedef struct {
    ...
    XVmul2_uio_map maps[ MAX_UIO_MAPS ];
} XVmul2_uio_info;

/***************** Variable Definitions **************************************/
static XVmul2_uio_info uio_info;
...
static int uio_info_read_name(XVmul2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVmul2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVmul2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVmul2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVmul2_Initialize(XVmul2 *InstancePtr, const char* InstanceName) {
	XVmul2_uio_info *InfoPtr = &uio_info;
