...
typedef struct {
    ...
    XVadd2_uio_map maps[ MAX_UIO_MAPS ];
} XVadd2_uio_info;

/***************** Variable Definitions **************************************/
static XVadd2_uio_info uio_info;
...
static int uio_info_read_name(XVadd2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XVadd2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XVadd2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XVadd2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XVadd2_Initialize(XVadd2 *InstancePtr, const char* InstanceName) {
	XVadd2_uio_info *InfoPtr = &uio_info;
