...
struct drain : public ff_node {
    ...
    void * svc(void * t)
    {
        if (t) {
            ...
            int * out = (int *)(task->outputs[0].ptr);
            ...
            for(int i=0; i<task->outputs[0].size/sizeof(int); i++) 
             std::cout << "Sum : " << out[i] << " i"<< i <<std::endl;
            std::cout << "\n "<< std::endl;
        	...
        	if (worker!=0) worker=max_krnl;
      		...
      		for (int i = 0; i < worker; ++i) {
           	  ...
           	  /*p->add_stage(new generator(n, m, input_l_vinc, output_l_vinc, mem_offset_vinc, i));
                  ...
                  //********* n vadd ************************
                  p->add_stage(new generator(n, m, input_l_vadd, output_l_vadd, mem_offset_vadd, i));
                  p->add_stage(new FNodeTask(devices[j], kernel_vadds[i], chain));
                  p->add_stage(new drain(input_l_vadd, output_l_vadd, i));
                  p->cleanup_nodes();
           	  w.push_back(p);
        	}
