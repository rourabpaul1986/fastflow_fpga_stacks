...
FTask * new_task(int n, int i_l, int o_l, int mem_offset, int kernel_id)
{
    ...
    ret |= posix_memalign((void **)&a[0], 4096, size_in_bytes);
    ret |= posix_memalign((void **)&b[1], 4096, size_in_bytes);
    ...
    *s = n;
    ...
    task->add_input(a,  size_in_bytes, (kernel_id*(i_l+o_l))+mem_offset);
    task->add_output(c,  size_in_bytes, (kernel_id*(i_l+o_l))+1+mem_offset);
    //for (int i = 0; i < i_l; ++i) 
...
FTask * new_task_middle(int n, int i_l, int o_l, int mem_offset, int kernel_id, int * cc)
{
    ...
    *s = n;
    ...
    FTask * task = new FTask();
    task->add_input(a,  size_in_bytes, (kernel_id*(i_l+o_l))+mem_offset);
    task->add_input(b,  size_in_bytes, (kernel_id*(i_l+o_l))+1+mem_offset);
    task->add_output(c, size_in_bytes, (kernel_id*(i_l+o_l))+2+mem_offset);
    //   for (int i = 0; i < i_l; ++i) 
    //task->add_input(in[i],  size_in_bytes, (kernel_id*(i_l+o_l))+i+mem_offset );
    // for (int i = 0; i < o_l; ++i) 
    //task->add_output(out[i], size_in_bytes, (kernel_id*(i_l+o_l))+i+i_l+mem_offset);
...
public:
    ...
    middle(int n,  int i_l, int o_l, int mem_offset, int index)
    : n(n)
    , i_l(i_l)
    , o_l(o_l)
    , mem_offset(mem_offset)
...
int main(int argc, char * argv[])
{
    ...
    if (nWorkers == 0) {
        ...
        ff_pipeline p;
        p.add_stage(new generator(n, m, input_l_vinc, output_l_vinc, mem_offset_vinc,3));
        p.add_stage(new FNodeTask(device, kernel_vincs[3], chain));
        ...
        //p.add_stage(new middle(n, input_l_vinc, output_l_vinc, mem_offset_vinc, 1));
        //p.add_stage(new FNodeTask(device, kernel_vincs[1], chain));
        //p.add_stage(new drain(input_l_vinc, output_l_vinc));
        
        p.add_stage(new middle(n, input_l_vadd, output_l_vadd, mem_offset_vadd, 1));
        p.add_stage(new FNodeTask(device, kernel_vadds[1],chain));
        p.add_stage(new drain(input_l_vadd, output_l_vadd));
